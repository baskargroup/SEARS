...
export function UpdateProductInfo({rowData, label, icon}) {
        ...
        try {
            ...
            for (let i = 0; i < file?.[typeVal]?.length || 0; i++) {
                ...
                try{
                    ...
                    await sendLogToServer('error', 'Error during file upload : ' + e?.message || '')
                    return false;
                }
            ...
            await sendLogToServer('error', 'Error in updating product info : ' + e?.message || '')
            return false;
        }
    };
    ...
    const fetchDownloadLink = async (filePath) => {
        try {
            ...
            await sendLogToServer('error', 'Error in fetching download link  : ' + e?.message || '')
    ...
    const handleMobilityChange = (e, index, field) => {
        const newMobility = [...formData.mobility];
        if(field === 'batch'){
        newMobility[index].batch = e.target.value;
        }

        if(field === 'reading'){
            newMobility[index].reading = e.target.value;
            }

        if(field === 'who'){
                newMobility[index].who = e.target.value;
                }

        setFormData({ ...formData, mobility: newMobility });
    };


    // Add a new mobility field
    const addMobility = () => {
        setFormData({
            ...formData,
            mobility: [...formData.mobility, { batch: '', reading: '', who: '' }],
        });
    };
        ...
        <div>
                ...
                <TabView scrollable>
                    <TabPanel header="Metadata">
                        <div>
                            ...
                            <div className="field">
                                <label htmlFor="exposure_time" className="font-bold">
                                    Exposure Time
                                </label><br />
                                <InputText id="exposure_time" value={formData?.exposure_time}  />
                            </div>
            ...
            <div className="button-container">
              ...
              {/* <Button label="Submit" onClick={() => saveProduct(allTypes[1])} /> */}
                    ...
                    <TabPanel header="Mobility">
                        <div>
                            {formData?.mobility.map((singleMobility, idx) => (
                                 <div className="container" key={idx}> 
                                 <div className="field"> 
                                     <span className="text">Batch ID</span>
                                     <textarea className="textbox" value={formData?.mobility[idx]?.batch} onChange={(e) => handleMobilityChange(e,idx,'batch')} rows={1} cols={30}/>
                                 
                                     <span className="text">Reading</span>
                                     <textarea className="textbox" value={formData?.mobility[idx]?.reading} onChange={(e) => handleMobilityChange(e,idx,'reading')} rows={1} cols={30}/>
                                     
                                     <span className="text">Entry Person</span>
                                     <textarea className="textbox" value={formData?.mobility[idx]?.who} onChange={(e) => handleMobilityChange(e,idx,'who')} rows={1} cols={30}/>
                                 </div>
                             </div>
                            ...
                            <div className="button-container">
                                <div className="butt">
                                    <Button label="Add a new mobility reading" onClick={addMobility} />
                                </div>
                                <div  className="butt">
                                    <Button className="butt" label="Save Data" onClick={()=>saveProduct(allTypes[0])}  />
                                </div>
                    ...
                    <TabPanel header="FAIR DATA">
                        {/* <div>
                           
                         <p>print</p>
                         {formData?.thickness.map((data, idx) => (<div key={idx}>{formData?.thickness[idx]?.reading}</div>))}

                        </div> */}
                        <div className = "container">
                        <div style = {{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 50, textAlign : "center"}}>
                            <p>Download the data in FAIR format</p>
                            <React.Fragment>
                                <DownloadData rowData={formData} />
                            </React.Fragment>
                        </div>
